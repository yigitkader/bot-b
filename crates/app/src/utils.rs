...
pub fn get_qty_step(symbol_rules: Option<&std::sync::Arc<SymbolRules>>, fallback: f64) -> f64 {
    symbol_rules
...
pub fn get_price_tick(symbol_rules: Option<&std::sync::Arc<SymbolRules>>, fallback: f64) -> f64 {
    symbol_rules
...
pub fn is_usd_stable(asset: &str) -> bool {
    matches!(asset.to_uppercase().as_str(), "USDT" | "USDC")
}
...
pub fn calc_qty_from_margin(
    ...
    let (price_quantized, _effective_tick_size) = if tick_size >= price {
        ...
        let quantized = match side {
            Side::Buy => quant_utils_floor_to_step(price, calculated_tick),
            Side::Sell => quant_utils_ceil_to_step(price, calculated_tick),
        };
        (quantized, calculated_tick)
    } else {
        let quantized = match side {
    ...
    let price_diff_bps = if price > Decimal::ZERO {
        ((price_quantized - price).abs() / price * Decimal::from(10000))
            .to_f64()
            .unwrap_or(0.0)
    } else {
        0.0
...
pub fn required_take_profit_price(
    ...
    let fee_entry =
        notional_entry * Decimal::try_from(fee_bps_entry / 10_000.0).unwrap_or(Decimal::ZERO);
...
pub fn adjust_price_for_aggressiveness(
    ...
    match side {
        crate::types::Side::Buy => {
            ...
            let adjustment_factor = if trend_factor < opportunity_factor {
                trend_factor
            } else {
                opportunity_factor
            };
            let adjustment_factor_dec =
                Decimal::try_from(adjustment_factor).unwrap_or(Decimal::ONE);
            ...
            if requests.len() >= self.max_requests_per_sec as usize {
                if let Some(oldest) = requests.front().copied() {
                    ...
                    if wait_time > now {
                        ...
                        self.sleep_with_polling(sleep_duration, poll_interval_ms)
                            .await;
                        continue;
                    }
                }
...
pub fn should_place_trade(
    ...
    let risk_reward_ratio =
        profit_guarantee.calculate_risk_reward_ratio(spread_bps, position_size_usd, max_loss_pct);
...
pub fn apply_fill_rate_decay(state: &mut SymbolState, cfg: &crate::config::AppCfg) {
    ...
    if state.active_orders.is_empty() {
        ...
        state.order_fill_rate = (state.order_fill_rate
            * cfg.internal.fill_rate_slow_decrease_factor
            + cfg.internal.fill_rate_slow_decrease_bonus)
            .min(1.0);
    }
}
...
pub fn validate_quotes(
    ...
    if let Some((px, q)) = quotes.bid {
        if px.0 <= Decimal::ZERO {
            ...
            let quantized_to_zero =
                qty_step_dec > Decimal::ZERO && nq.0 < qty_step_dec && nq.0 != Decimal::ZERO;
...
pub fn adjust_quotes_for_risk(
    ...
    match risk_action {
        RiskAction::Reduce => {
            ...
            let widen =
                Decimal::from_f64_retain(order_price_distance_no_position).unwrap_or(Decimal::ZERO);
            quotes.bid = quotes
                .bid
...
pub async fn place_side_orders(
    ...
    info!(
        ...
        "DEBUG: place_side_orders function entry"
    );

    if let Some(quote) = quote {
        // ✅ KRİTİK: with_order_lock'u burada KULLANMA - zaten processor.rs'de lock alınmış
        // Çift lock deadlock yaratır (Tokio mutex'ler reentrant değildir)
        // Lock zaten processor.rs'deki with_order_lock içinde alınmış
        ...
        let position_chunks_count = {
            ...
            let position_matches_side = (side == Side::Buy && is_position_long)
                || (side == Side::Sell && is_position_short);
        ...
        let available_margin =
            (caps.buy_total - *total_spent_on_side - total_spent_on_other).max(0.0);

        // ✅ DEBUG: place_side_orders başlangıcında logla
        info!(
            %symbol,
            side = ?side,
            available_margin,
            buy_total = caps.buy_total,
            total_spent_on_side = *total_spent_on_side,
            total_spent_on_other,
            open_orders,
            position_chunks_count,
            total_open_chunks,
...
pub fn process_order_canceled(
    ...
    if should_remove {
        ...
        json_logger.log_order_canceled(
            symbol,
            order_id,
