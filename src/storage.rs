...
impl StateStorage {
    ...
    pub async fn save_ordering_state(&self, state: &OrderingState) -> Result<()> {
        ...
        let order_ts = state.last_order_update_timestamp.and_then(|ts| {
        ...
        let pos_ts = state.last_position_update_timestamp.and_then(|_| {
            let now_duration = SystemTime::now()
                .duration_since(UNIX_EPOCH)
                .unwrap_or_default();
            Some(now_duration.as_nanos() as i64)
        ...
        db.execute(
            ...
            params![
                ...
                state
                    .open_position
                    .as_ref()
                    .map(|p| p.entry_price.0.to_string()),
    ...
    pub async fn restore_order_fill_history(&self) -> Result<Vec<(String, OrderFillHistory)>> {
        ...
        let rows = stmt.query_map([], |row| {
            ...
            let total_filled_qty = Decimal::from_str(&total_filled_qty_str).map_err(|e| {
                rusqlite::Error::InvalidColumnType(
                    0,
                    format!("Failed to parse qty: {}", e),
                    rusqlite::types::Type::Text,
                )
            })?;
            let weighted_price_sum = Decimal::from_str(&weighted_price_sum_str).map_err(|e| {
                rusqlite::Error::InvalidColumnType(
                    0,
                    format!("Failed to parse price: {}", e),
                    rusqlite::types::Type::Text,
                )
            })?;
        ...
        info!(
            count = history.len(),
            "STORAGE: Restored {} order fill history entries from database",
            history.len()
        );
        Ok(history)
    }
    ...
    pub async fn log_balance_reservation(
        ...
        db.execute(
            ...
            "#,
            params![asset, amount.to_string(), now, order_id, symbol,],
        )?;
    ...
    pub async fn cleanup_balance_reservation_logs(&self, days: u32) -> Result<usize> {
        ...
        info!(
            deleted_count = count,
            days,
            "STORAGE: Cleaned up {} old balance reservation logs (older than {} days)",
            count,
            days
        );
        Ok(count)
    ...
    pub async fn cleanup_old_fill_history(&self, days: u32) -> Result<usize> {
        ...
        info!(
            deleted_count = count,
            days,
            "STORAGE: Cleaned up {} old order fill history entries (older than {} days)",
            count,
            days
        );
        Ok(count)
...
impl Storage {
    ...
    pub async fn start(&self) -> Result<()> {
        ...
        tokio::spawn(async move {
            ...
            info!(
                "STORAGE: Started, listening to OrderFillHistoryUpdate events (with batch writing)"
            );
        ...
        match update.action {
            crate::event_bus::FillHistoryAction::Save => {
                if let Some(data) = &update.data {
                    ...
                    storage
                        .save_order_fill_history(&update.order_id, &update.symbol, &history)
                        .await?;
