...
impl SymbolScanner {
    ...
    pub fn score_symbols(&self, metrics: &HashMap<String, SymbolMetrics>) -> Vec<SymbolScore> {
        if metrics.is_empty() {
            return Vec::new();
        }
        ...
        let max_spread = metrics.values().map(|m| m.spread_bps).fold(0.0, f64::max);
        ...
        for (symbol, metric) in metrics {
            ...
            let total_score = volatility_score * self.config.volatility_weight
    ...
    pub async fn run_rotation_task(
        &self,
        rotation_tx: tokio::sync::broadcast::Sender<crate::types::SymbolRotationEvent>,
    ) {
        use chrono::Utc;
        use std::collections::HashSet;

        let mut interval = tokio::time::interval(tokio::time::Duration::from_secs(
            self.config.rotation_interval_minutes * 60,
        ));
        ...
        loop {
            ...
            let old_symbols: HashSet<String> = self.get_selected_symbols().await.into_iter().collect();

            match self.select_symbols().await {
                Ok(new_symbols) => {
                    let new_symbols_set: HashSet<String> = new_symbols.iter().cloned().collect();
                    let removed: Vec<String> = old_symbols
                        .difference(&new_symbols_set)
                        .cloned()
                        .collect();
                    let added: Vec<String> = new_symbols_set
                        .difference(&old_symbols)
                        .cloned()
                        .collect();

                    info!(
                        "SYMBOL_SCANNER: Rotation completed, {} symbols selected (added: {}, removed: {})",
                        new_symbols.len(),
                        added.len(),
                        removed.len()
                    );
                    ...
                    if !added.is_empty() || !removed.is_empty() {
                        let event = crate::types::SymbolRotationEvent {
                            new_symbols: added,
                            removed_symbols: removed,
                            all_symbols: new_symbols.clone(),
                            ts: Utc::now(),
                        };
                        if rotation_tx.send(event).is_err() {
                            warn!("SYMBOL_SCANNER: Failed to send rotation event (no receivers)");
                        }
                    }
                }
                Err(err) => {
...
impl SymbolSelectionConfig {
    pub fn from_file_config(
        ...
        Self {
            enabled: selection_cfg.and_then(|s| s.enabled).unwrap_or(false),
            max_symbols: selection_cfg.and_then(|s| s.max_symbols).unwrap_or(30),
            ...
            min_trades_24h: selection_cfg.and_then(|s| s.min_trades_24h).unwrap_or(1000),
            volatility_weight: selection_cfg
                .and_then(|s| s.volatility_weight)
                .unwrap_or(2.0),
            volume_weight: selection_cfg.and_then(|s| s.volume_weight).unwrap_or(1.0),
            trades_weight: selection_cfg.and_then(|s| s.trades_weight).unwrap_or(0.5),
            spread_weight: selection_cfg.and_then(|s| s.spread_weight).unwrap_or(1.0),
            allowed_quotes,
        }
    }
