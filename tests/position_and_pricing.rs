...
async fn test_position_id_different_symbols() {
    ...
    assert_ne!(id1, id2, "Position IDs should be different for different symbols");
}
...
async fn test_position_id_different_sides() {
    ...
    assert_ne!(id1, id2, "Position IDs should be different for different sides");
}
...
fn calculate_maker_limit_price(
    side: &Side,
    best_bid: f64,
    best_ask: f64,
    tick_size: f64,
...
async fn test_limit_order_pricing_maker() {
    ...
    assert!(long_price <= best_ask, "LONG maker order should be <= best_ask, got {}", long_price);
    assert!(long_price > best_bid, "LONG maker order should be > best_bid, got {}", long_price);
    ...
    assert!(short_price >= best_bid, "SHORT maker order should be >= best_bid, got {}", short_price);
    assert!(short_price < best_ask, "SHORT maker order should be < best_ask, got {}", short_price);
}
...
async fn test_limit_order_pricing_aggressive() {
    ...
    assert_eq!(long_price_aggressive, best_ask, "Aggressive LONG should use best_ask");
    
    // Aggressive SHORT: uses best_bid (crosses spread, immediate fill)
    let short_price_aggressive = best_bid;
    assert_eq!(short_price_aggressive, best_bid, "Aggressive SHORT should use best_bid");
}
...
async fn test_tick_size_rounding() {
    ...
    assert!(remainder.abs() < 1e-10, "Price should be rounded to tick size, remainder: {}", remainder);
...
async fn test_zero_tick_size() {
    ...
    let long_price = calculate_maker_limit_price(&Side::Long, best_bid, best_ask, tick_size);
    assert_eq!(long_price, best_bid, "With zero tick size, should use best_bid");
    
    let short_price = calculate_maker_limit_price(&Side::Short, best_bid, best_ask, tick_size);
    assert_eq!(short_price, best_ask, "With zero tick size, should use best_ask");
}
